...
public class KeyLoader {
    ...
    public ECKey loadPrivateKey() throws ParseException {
        String privateKeyJson = keyProperties.getPrivateKey();
        ...
        ECKey key = (ECKey) JWK.parse(privateKeyJson);
        if (key.getD() == null) {
    ...
    public ECKey loadPublicKey() throws ParseException {
        String publicKeyJson = keyProperties.getPublicKey();

        if (publicKeyJson == null || publicKeyJson.trim().isEmpty()) {
        ...
        log.info("Loading public key from backend: {}", publicKeyJson);
        return (ECKey) JWK.parse(publicKeyJson);
    }
}
